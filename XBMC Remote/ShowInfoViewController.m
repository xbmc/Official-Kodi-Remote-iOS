...
-(IBAction)scrollDown:(id)sender{
    ...
    int bottom_scroll = (height_content - height_bounds)>0 ? height_content - height_bounds : 0;
...
//        [scrollView addSubview:recordButton];
    ...
    if (!([[item objectForKey:@"family"] isEqualToString:@"broadcastid"] || [[item objectForKey:@"family"] isEqualToString:@"recordingid"])){
        ...
        if ([[item objectForKey:@"clearlogo"] length] != 0){
            ...
            NSString *stringURL = [NSString stringWithFormat:@"http://%@%@", serverURL, [[item objectForKey:@"clearlogo"] stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLFragmentAllowedCharacterSet]]];
    ...
    if (height_content > height_bounds-height_navbar){
...
- (void) scrollViewDidScroll: (UIScrollView *) theScrollView{
    ...
    if (arrow_continue_down.hidden==NO && at_bottom==YES){
        arrow_continue_down.hidden=YES;
    ...
    else if (arrow_continue_down.hidden==YES && at_bottom==NO){
        arrow_continue_down.hidden=NO;
        [UIView beginAnimations:nil context:nil];
        [UIView setAnimationCurve:UIViewAnimationCurveEaseOut];
        [UIView setAnimationDuration:1];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSString *stringURL = [NSString stringWithFormat:@"http://%@%@", serverURL, [[[cast objectAtIndex:indexPath.row] objectForKey:@"thumbnail"] stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLFragmentAllowedCharacterSet]]];
